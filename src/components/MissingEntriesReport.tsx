...
export const MissingEntriesReport: React.FC<Props> = ({ report, onRefresh, isLoading, selectedMonth }) => {
  ...
  const filteredReport = report.filter(item => {
    ...
    const matchesTsm = !selectedTsm || item.tsm === selectedTsm;
    
    return matchesSearch && matchesFilter && matchesTsm;
  });
    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        ...
        <div className="flex items-center gap-2 w-full md:w-auto">
          {selectedTsm && (
            <button 
              onClick={() => setSelectedTsm(null)}
              className="px-3 py-2 bg-rose-100 text-rose-600 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-rose-200 transition-all flex items-center gap-2"
            >
              <span>{selectedTsm}</span>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div 
          className={`card-clean p-4 cursor-pointer transition-all ${selectedTsm === sortedTsmSummary[0]?.tsm ? 'bg-amber-100 border-amber-300 ring-2 ring-amber-400' : 'bg-amber-50 border-amber-100 hover:border-amber-200'}`}
          onClick={() => setSelectedTsm(selectedTsm === sortedTsmSummary[0]?.tsm ? null : sortedTsmSummary[0]?.tsm)}
        >
          <div className="flex items-center gap-3">
            ...
            <table className="w-full text-left">
              ...
              <tbody className="divide-y divide-slate-50">
                {sortedTsmSummary.map(item => (
                  <tr 
                    key={item.tsm} 
                    className={`cursor-pointer transition-colors ${selectedTsm === item.tsm ? 'bg-seablue/10' : 'hover:bg-slate-50/50'}`}
                    onClick={() => setSelectedTsm(selectedTsm === item.tsm ? null : item.tsm)}
                  >
                    <td className="px-4 py-3 text-[10px] font-bold text-slate-700">
                      <div className="flex items-center gap-2">
                        {selectedTsm === item.tsm && <div className="w-1.5 h-1.5 rounded-full bg-seablue animate-pulse" />}
                        {item.tsm}
                      </div>
                    </td>
